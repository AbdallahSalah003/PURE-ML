...
def plot_learning_curves(summary_df, arrays: dict, y_train, lr_svm_scenarios: dict, train_sizes=np.linspace(0.1, 1.0, 8)):
    CV = StratifiedKFold(n_splits=CV_FOLDS, shuffle=True, random_state=RANDOM_STATE)
    top4 = summary_df.nlargest(4, "cv_f1")["experiment"].tolist()
    ...
    plt.suptitle(
        "Learning curves - top 4 models by CV F1 score",
        fontsize=13
    )
    plt.tight_layout()
